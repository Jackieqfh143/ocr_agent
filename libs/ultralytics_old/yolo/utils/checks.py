...
from libs.ultralytics_old.yolo.utils import (AUTOINSTALL, LOGGER, ONLINE, ROOT, USER_CONFIG_DIR, TryExcept, clean_url, colorstr,
...
def check_latest_pypi_version(package_name='ultralytics_old'):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the ultralytics_old package is available on PyPI.
        ...
        with contextlib.suppress(Exception):
            ...
            if pkg.parse_version(__version__) < pkg.parse_version(latest):  # update is available
                LOGGER.info(f'New https://pypi.org/project/ultralytics/{latest} available 😃 '
                            f"Update with 'pip install -U ultralytics_old'")
                return True
    return False
