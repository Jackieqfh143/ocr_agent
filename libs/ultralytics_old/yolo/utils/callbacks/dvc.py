# libs.ultralytics_old YOLO 🚀, GPL-3.0 license
import os

import pkg_resources as pkg

from libs.ultralytics_old.yolo.utils import LOGGER, TESTS_RUNNING
...
def _logger_disabled():
    return os.getenv('libs.ultralytics_DVC_DISABLED', 'false').lower() == 'true'
...
def on_pretrain_routine_start(trainer):
    try:
        ...
        if not _logger_disabled():
            ...
            LOGGER.info(
                'DVCLive is detected and auto logging is enabled (can be disabled with `libs.ultralytics_DVC_DISABLED=true`).'
            )
        else:
            LOGGER.debug('DVCLive is detected and auto logging is disabled via `libs.ultralytics_DVC_DISABLED`.')
            live = None
