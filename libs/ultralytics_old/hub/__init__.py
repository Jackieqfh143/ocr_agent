...
from libs.ultralytics_old.hub.auth import Auth
from libs.ultralytics_old.hub.utils import PREFIX
from libs.ultralytics_old.yolo.data.utils import HUBDatasetStats
from libs.ultralytics_old.yolo.utils import LOGGER, SETTINGS, USER_CONFIG_DIR, yaml_save
...
def login(api_key=''):
    ...
    Example:
        from ultralytics_old import hub
        hub.login('API_KEY')
    """
...
def start(key=''):
    ...
    LOGGER.warning(f"""
WARNING ⚠️ ultralytics_old.start() is deprecated after 8.0.60. Updated usage to train Ultralytics HUB models is:

from ultralytics_old import YOLO, hub
...
def check_dataset(path='', task='detect'):
    ...
    Example:
        ```python
        from ultralytics_old.hub import check_dataset
