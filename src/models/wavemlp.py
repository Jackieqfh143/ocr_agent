...
class WaveNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.fork_feat:
            return x
        x = self.norm(x)
        out = F.adaptive_avg_pool2d(x,output_size=1).flatten(1)
...
if __name__ == '__main__':
    device = "cpu"
    model_path = "/home/codeoops/code/ocr_agent/weights/WaveMLP_S.pth.tar"
    model = WaveMLP_S()
    x = torch.Tensor(1,3,224,224).to(device)
    y = model(x)
